...
toxpi_results <- read.csv(input_file_path1)
...
data <- read.csv(input_file_path2)
...
output_plot <- ggplot(rankDF, aes(x = gaRank, y = Rank, color = Source)) + 
  geom_point(size = 2) + 
  theme_bw() +
...
ggsave("Figure_10.png", plot = output_plot)